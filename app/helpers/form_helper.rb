module FormHelper
  ...
  def form_group_for( form, field,opts={},  &block )
    ...
    has_errors = form.object.error[field].present? content_tag :div, class: 'form-group#{'has_error' if has_error }' do
      concat form.label(field, class: 'control-label' )
      if label concat capture(&block) concat errors_for(form, field)
    end
  end
end
